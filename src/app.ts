...
export class DeckHelperApp {
  ...
  private renderInventory() {
    ...
    const ownedMap = new Map(this.state.inventory.cards.map((card) => [card.cardName, card] as const))
    const filteredCards = cards.filter((card) => {
      if (this.ownedOnly && !ownedMap.has(card.name)) return false
    ...
    return `
      <section class="page-head">
        <div><h2>Inventory</h2><p>Add only the cards and aura variants you actually own. Everything saves automatically in this browser.</p></div>
      </section>
      <section class="panel inventory-panel">
        <div class="toolbar">
          ...
          <span class="muted">${filteredCards.length} shown</span>
        </div>
        <div class="card-list">
          ${filteredCards.map((card) => this.renderInventoryCard(card, ownedMap.get(card.name))).join('')}
        </div>
      </section>
  ...
  private renderInventoryCard(card: (typeof cards)[number], owned?: OwnedCard) {
    const image = thumbnail(card.imageAssetId)
    return `
      <article class="inventory-row ${owned ? 'owned' : ''}">
        <div class="thumb">${image ? `<img src="${escapeHtml(image)}" alt="">` : '<span>?</span>'}</div>
        <div class="item-main">
          <strong>${escapeHtml(card.name)}</strong>
          <small>${escapeHtml(card.ability || 'No ability')} · 1/${formatCompact(card.rarity)}</small>
        </div>
        ${owned ? `
          ...
          <button class="danger subtle" data-action="remove-card" data-name="${escapeHtml(card.name)}">Remove</button>
        ` : `<button class="primary compact" data-action="add-card" data-name="${escapeHtml(card.name)}">Add</button>`}
      </article>
    `
  }
  ...
  private renderOptimize() {
    ...
    return `
      ...
      <section class="lock-summary panel">
        <div><span>Locked cards</span><strong>${lockedCards.length ? lockedCards.map((card) => `${escapeHtml(card.cardName)}${card.lockedPosition !== null ? ` (#${card.lockedPosition + 1})` : ''}`).join(', ') : 'None'}</strong></div>
      ...
      <section class="replacement-section">
        ...
        <div class="replacement-list">${this.replacementResults.map((result, index) => `
          <article class="replacement-card">
            <b>#${index + 1}</b>
            <div><strong>${escapeHtml(result.cardName)}</strong><span>${escapeHtml(deckLabel(result.loadout))}</span></div>
  ...
  private onClick(event: Event) {
    ...
    else if (action === 'remove-card') this.removeCard(target.dataset.name || '')
  ...
  private removeCard(name: string) {
    this.state.inventory.cards = this.state.inventory.cards.filter((card) => card.cardName !== name)
    this.state.currentDeck.cards = this.state.currentDeck.cards.filter((card) => card.cardName !== name)
    this.persist(); this.render()
  }

  private toggleCardBorder(name: string, border: BorderName, checked: boolean) {
  ...
  private toggleCardLock(name: string, checked: boolean) {
    const card = this.state.inventory.cards.find((entry) => entry.cardName === name)
    if (!card) return
    card.locked = checked
  ...
  private setCardPosition(name: string, value: string) {
    const card = this.state.inventory.cards.find((entry) => entry.cardName === name)
    if (!card) return
  ...
  private setDeckCard(slot: DeckSlot, name: string) {
