...
const auraStyles = readFileSync('src/aura-workspace.css', 'utf8')
...
assert.ok(app.includes('Depth Bans') && app.includes('depth-ban-add') && app.includes('depth-ban-remove'), 'Optional Depth ban controls are missing')
assert.ok(app.includes('bannedCardNames: [...this.state.depthBans]'), 'Optimizer requests must include selected Depth bans')
assert.ok(auraStyles.includes('.depth-ban-panel') && auraStyles.includes('.depth-ban-suggestions'), 'Depth ban styling is missing')
